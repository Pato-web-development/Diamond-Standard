...
contract SafeFinance is ERC20, ERC20Burnable, ERC20Snapshot, Ownable, Pausable, ERC20Permit, ERC20Votes {
    constructor() ERC20("Safe Finance", "SFC2") ERC20Permit("Safe Finance") {
        _mint(address(this), 100000000 * 10 ** decimals());
    }

    uint public amountToReceive;

    event Received(address indexed sender, uint amount);

    function receiveEther(uint _amountToReceive) {
          amountToReceive = _amountToReceive;
    }

    receive() external payable {
        require(amountToReceive >= (3 * 15 ** decimals()), "not up to required amount");
        emit Received(msg.sender, amountToReceive);
    }
    ...
    function buyTokenWithToken(address _tokenContractAddress, uint256 _tokenAmount) internal {
        ...
        _mint(msg.sender, mintMyTokenAmount);
    }
   ...
   function transferToken(address _tokenContractAddress, address _receivingAddress, uint256 _tokenAmount) public onlyOwner {
        IERC20(_tokenContractAddress).transferFrom(address(this), _receivingAddress, _tokenAmount);
   }
